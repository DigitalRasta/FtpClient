...
CurlMulti &CurlMulti::addHandle(const CurlEasy &handler) throw() {
...
CurlMulti &CurlMulti::addHandle(const vector<CurlEasy> &handlers) throw() {
    if (this->curl!=nullptr) {
...
CurlMulti &CurlMulti::removeHandle(const CurlEasy &handler) throw() {
    if (this->curl!=nullptr) {
...
const int CurlMulti::getActiveTransfers() const throw() {
    return this->active_transfers;
}
    
// Implementation of getMessageQueued method
const int CurlMulti::getMessageQueued() const throw() {
...
const string CurlMulti::toString(const CURLMcode code) const throw() {
