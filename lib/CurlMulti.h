...
namespace curl {
    ...
    protected:
        const string toString(const CURLMcode) const throw();
    public:
        CurlMulti();
        ...
        CurlMulti &addHandle(const CurlEasy &) throw();
        CurlMulti &addHandle(const vector<CurlEasy> &) throw();
        CurlMulti &removeHandle(const CurlEasy &) throw();
        int perform();
        const int getActiveTransfers() const throw();
        const int getMessageQueued() const throw();
