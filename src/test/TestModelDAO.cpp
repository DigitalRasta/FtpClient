...
#include <definition\ModelConnection.cpp>
#include <vector>
#include <string>
	...
	TEST_CLASS(TestModelDAO) {
		...
		public:
			...
			TEST_METHOD(getDirContentNormalWork) {
				...
				Assert::AreEqual(14, element.day);
				Assert::AreEqual(4, element.month);
				Assert::AreEqual(2014, element.year);
				Assert::AreEqual(12, element.hour);
				Assert::AreEqual(1, element.minute);
			...
			TEST_METHOD(createConnectionAndGetConnection) {
				int firstConnectionID = this->modelObject->createNewConnection("Random host", "random port", "random login", "random password");
				int secondConnectionID = this->modelObject->createNewConnection("host", "port", "login", "password");
				Assert::AreEqual(0, firstConnectionID);
				Assert::AreEqual(1, secondConnectionID);
			}
	};
}
